...
import { Deal, fmtMoneyK } from '@/lib/types'
  ...
  return (
    <div className="flex flex-col min-w-0">
      ...
      <div className="flex items-center gap-2 px-1 pb-3">
        <span className="w-[7px] h-[7px] rounded-full flex-shrink-0" style={{ background: isWon ? 'var(--accent)' : 'rgba(255,255,255,0.28)' }} />
        <span className="text-[13px] font-[600]">{stage.label}</span>
        <span className="text-[12px] font-[500]" style={{ color: 'var(--t4)' }}>{deals.length}</span>
        <span className="tnum ml-auto text-[12px] font-[500]" style={{ color: 'var(--t3)' }}>{fmtMoneyK(sum)}</span>
      </div>
      ...
      <div ref={setNodeRef} className="flex-1 flex flex-col gap-2.5 p-2 rounded-[14px] min-h-[120px] transition-all"
        ...
        {deals.length === 0 && (
          <div className="flex-1 grid place-items-center text-[12px]" style={{ color: 'var(--t4)' }}>Drop here</div>
        )}
      </div>
    </div>
  )
