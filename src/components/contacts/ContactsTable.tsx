...
import { Contact, fmtMoney } from '@/lib/types'
...
const COLS = [
  { key: 'name',    label: 'Name',          w: '26%' },
  { key: 'type',    label: 'Type',          w: '13%' },
  { key: 'company', label: 'Company',       w: '20%' },
  { key: 'value',   label: 'Account value', w: '16%', num: true },
  { key: 'owner',   label: 'Owner',         w: '11%' },
  { key: 'created', label: 'Added',         w: '14%' },
]
    ...
    <div>
      ...
      <div className="flex gap-3 mb-[18px] items-center flex-wrap">
        <div className="relative flex-1 min-w-[240px] max-w-[360px]">
          ...
          <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search name or company…"
        ...
        <div className="flex gap-1 p-1 rounded-[11px]" style={{ background: 'var(--s1)', boxShadow: 'inset 0 0 0 1px var(--line)' }}>
          {(['all','lead','prospect','customer'] as const).map(k => (
            <button key={k} onClick={() => setFilter(k)}
              className="px-[13px] py-[7px] rounded-lg text-[13px] font-[500] transition-all capitalize"
              ...
              <span className="ml-[7px] text-[12px]" style={{ color: 'var(--t4)' }}>{counts[k]}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <Card pad={0} style={{ overflow: 'hidden' }}>
        ...
        <div className="grid px-[22px]" style={{ gridTemplateColumns: COLS.map(c => c.w).join(' '), borderBottom: '1px solid var(--line)' }}>
          {COLS.map(c => (
            <button key={c.key} onClick={() => toggleSort(c.key)}
              ...
              {c.label}
              <Icon name="sort" size={13} style={{ opacity: sort.key === c.key ? 0.9 : 0.3, transform: sort.key === c.key && sort.dir < 0 ? 'scaleY(-1)' : 'none' }} />
            </button>
          ))}
        </div>
        {/* Rows */}
        <div>
          {filtered.map((c, i) => (
            <div key={c.id} className="row-btn group grid items-center px-[22px] h-[62px] transition-colors"
              ...
              <div className="flex items-center justify-between text-[13px]" style={{ color: 'var(--t3)' }}>
                <Link href={`/contacts/${c.id}`}>{new Date(c.created_at).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}</Link>
                <div className="flex items-center gap-1.5">
                  <button onClick={() => onDelete(c.id)} className="opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded hover:text-red-400"
                    style={{ color: 'var(--t4)' }} title="Delete">
          ...
          {filtered.length === 0 && (
            <div className="py-12 text-center text-[14px]" style={{ color: 'var(--t3)' }}>No contacts match your search.</div>
          )}
        </div>
      </Card>
    </div>
